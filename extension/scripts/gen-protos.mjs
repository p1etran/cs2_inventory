...
const SOURCES = [
  ...
  {
    file: 'node_modules/steam-user/protobufs/steammessages_clientserver_login.proto',
    // Steam's own hello, sent to open an anonymous connection before asking
    // for a QR session. Renamed because the game coordinator has a different
    // message by the same name, and this descriptor has one flat namespace.
    messages: [{ name: 'CMsgClientHello', as: 'CMsgClientHelloSteam' }],
  },
  {
    /*
     * The QR sign-in messages.
     *
     * steam-session ships these as a built protobufjs module rather than as
     * .proto files, so they are read out of its loaded schema by reflection.
     * That is still Valve's definition as shipped -- the alternative was
     * transcribing field numbers by hand, which this project has already been
     * bitten by more than once.
     */
    root: () => {
      const schema = require('steam-session/dist/protobuf-generated/load.js').default;
      // Its generated types belong to a real protobufjs Root, which resolves
      // the enums the request messages reference. The exported lookup map does
      // not -- it holds the message types only.
      const root = schema.CAuthentication_DeviceDetails.root;
      root.resolveAll();
      return root;
    },
    messages: [
...
/** Where a type was emitted from, so a name collision can name both sides. */
const emittedFrom = new Map();

for (const source of SOURCES) {
  const origin = source.file ?? 'steam-session schema';
  let root;
  if (source.root) {
    root = source.root();
  } else {
    ...
    root = rootForFile().loadSync(path.join(REPO, source.file), { keepCase: true });
    root.resolveAll();
  }
  ...
  for (const entry of source.messages) {
    ...
    if (!type) throw new Error(`${name} not found in ${origin}`);
    if (typeof entry !== 'string' && entry.as) renamed.set(type.fullName, entry.as);
    collect(type, seen);
  }
  ...
  for (const fullName of seen) {
    ...
    const emitAs = renamed.get(fullName) ?? fullName;
    /*
     * The descriptor is one flat namespace, so two messages sharing a name
     * would silently overwrite each other and every call site would encode
     * against whichever won. Steam and the game coordinator both define a
     * CMsgClientHello, so this is not hypothetical -- hence the rename above,
     * and hence failing loudly rather than trusting anyone to notice.
     */
    const key = emitAs.replace(/^\./, '');
    const previous = emittedFrom.get(key);
    if (previous && previous !== origin) {
      throw new Error(
        `${key} is defined in both ${previous} and ${origin}. ` +
          'Rename one with { name, as } -- a flat descriptor cannot hold both.',
      );
    }
    emittedFrom.set(key, origin);

    place(tree, emitAs, stripLongDefaults(type.toJSON()));
    count += 1;
  }
}
...
const exported = [
  ...new Set(
    SOURCES.flatMap((source) =>
      source.messages.map((entry) => (typeof entry === 'string' ? entry : (entry.as ?? entry.name))),
    ),
  ),
].sort();
